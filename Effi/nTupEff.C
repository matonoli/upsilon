...
void nTupEff(const Char_t *inputFile="upsE_0209.root", Int_t cutSet = 0, Int_t dataSet = 0, Int_t cMin = 0, Int_t cMax = 9, const Char_t *outFile="effiLM1S.txt") {
	...
	TH3F* hUpsPepP 	= new TH3F("hUpsPepP",";Upsilon p_{T};electron p;positron p",nbinspt,hleft,hright,150,0,15,150,0,15);	
	...
	for (int i = 0; i < nentries; ++i)
	{
		...
		for (int j = 0; j < nsyst; ++j)
    	{
			...
			h1ENom[1][j]->SetTitle("single e TPC acceptance;electron p;efficiency");
			hAccE[j]->Fill(upsPt);
			hNom[1][j]->Fill(upsPt);
			...
			h1ENom[5][j]->SetTitle("single e reconstruction;electron p;efficiency");
			hRecoE[j]->Fill(upsPt);
			hNom[3][j]->Fill(upsPt);
			...
			h1ENom[7][j]->SetTitle("single e track quality;electron p;efficiency");
			...
			h1ENom[9][j]->SetTitle("single e E/p;electron p;efficiency");
			hEpE[j]->Fill(upsPt);
			hNom[6][j]->Fill(upsPt);
	...
	for (int j = 0; j < nsyst; ++j)
	{
  	...
  	// fill flats
  	const float errAccT = 0.017;
  	const float errAccL = 0.03;
  	const float errTrackL = 0.058;
  	const float errTrackT = 0.058; 
  	...
  	for (int i = 1; i < nbinspt; ++i)
  	{
  		...
  		errT[i][0] = errAccT*errAccT + 4*errTrackT*errTrackT + 0.0094;
	...
	x1[0]=0; x1[1]=x1[0]; x1b[0]=cutnhitsf[cutSet]; x1b[1]=x1b[0];
  	...
  	for (int k = 0; k < nerrs; ++k)
  	{
  		for (int i = 1; i < nbinspt; ++i)
  		{
  			hSysL[k]->SetBinContent(i,errL[i][k]*100);
	...
	hNom[2][0]->Draw("same");
   	...
   	h1ENom[1][1]->SetTitle(";electron p;acceptance");
   	...
   	h1ENom[3][1]->SetTitle(";electron p;trigger efficiency");
   	...
   	h1ENom[5][1]->SetTitle(";electron p;track reconstruction");
   	...
   	h1ENom[7][1]->SetTitle(";electron p;track quality");
   	...
   	h1ENom[9][4]->SetTitle(";electron p;E/p cut efficiency");
   	...
   	h1ENom[11][7]->SetTitle(";electron p;BEMC matching efficiency");
