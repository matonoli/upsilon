...
//#include "/usr/src/linux-2.4/include/asm/msr.h"
#include "msr.h"
//#include "/usr/src/kernels/2.6.11-1.1369_FC4-i686/include/asm-i386/msr.h"
...
Quarkonium jpsi("jpsi");
Quarkonium ups("ups");

BemcTower       Quarkonium::bemcTower[4800];
BemcCluster     Quarkonium::bemcCluster[4800];
int             Quarkonium::daqIdFromSoftId[4801];
int             Quarkonium::bemcCtbMap[4800];
TrgDataType*    Quarkonium::trgData = 0;
unsigned short* Quarkonium::bemcData = 0;
int             Quarkonium::ctbKill[256];

void Quarkonium::init(int runNumber, int userInt[], float userFloat[])
{
  ...
  cout << "Quarkonium::init - Opening log file " << filename << endl;
  ...
  if (fUseCtb) {
    ...
    //for some odd reason, cons doesn't like memset
    for(int i=0; i<sizeof(ctbKill);i++){
      ctbKill[i]=0;
    }
    readCtbKillTable();
  }
...
bool Quarkonium::run(TrgDataType* trgData, unsigned short* bemcData)
{
  ...
  //
  Quarkonium::trgData = trgData;
  Quarkonium::bemcData = bemcData;

  vector<int> L0Seeds;
  vector<int> L2Seeds;
  findSeedTowers(L0Seeds, L2Seeds);

  printf("Number of L0 towers: %d\n", L0Seeds.size());
  printf("Number of L2 towers: %d\n", L2Seeds.size());

  cout << "L0: "<<endl;
  for (size_t i = 0; i < L0Seeds.size(); ++i){ 
     cout <<"soft ID = "<< bemcTower[L0Seeds[i]].softId << " DAQ Id = "<<L0Seeds[i]<<endl;
     L0candidates.push_back(bemcTower[L0Seeds[i]].softId);}
   cout << "L2: "<<endl;
   for (size_t i = 0; i < L2Seeds.size(); ++i) 
     cout <<"soft ID = "<< bemcTower[L2Seeds[i]].softId << " DAQ ID = "<<L2Seeds[i]<<endl;
  ...
  for (unsigned int i = 0; i < L0Seeds.size(); ++i) {
	...
	//
        // We have a candidate
	//
	mEnergyDaughter1 = bemcCluster[id1].E;
	mEnergyDaughter2 = bemcCluster[id2].E;
	mCosTheta = cosTheta;
	mInvMass = invMass;
	...
	hL2SeedTowers->fill(bemcTower[id2].softId);
        ...
        memcpy(&trgData->TrgSum.L2Result[L2RESULT_OFFSET], &l2result, sizeof(L2Result));
        return true;
#endif

	// Make sure the pair (id2,id1) is not already in the list
	// of candidates to avoid double-counting.
	pair<int, int> p(bemcTower[id1].softId, bemcTower[id2].softId);
	bool skip = false;
	for (size_t k = 0; k < candidates.size(); ++k) {
	  if (candidates[k].first == p.second && candidates[k].second == p.first) {
	    skip = true;
	    break;
	  }
	}
	if (!skip) {
	  ...
	  printf("id1=%d, id2=%d, m=%f, cos(theta)=%f, E1=%f, E2=%f\n",
		 bemcTower[id1].softId, bemcTower[id2].softId, invMass, cosTheta, bemcCluster[id1].E, bemcCluster[id2].E);
	}
      }
    }
  }
...
void Quarkonium::findSeedTowers(vector<int>& L0Seeds, vector<int>& L2Seeds)
{
  cout << "Quarkonium::findSeedTowers, Looking for towers above Thresholds" << endl;
  cout << "fL0SeedThreshold = " << fL0SeedThreshold << endl;
  cout << "fL2SeedThreshold = " << fL2SeedThreshold << endl;
  for (int daqId = 0; daqId < 4800; ++daqId) {
	...
	cout << "Found L2Seed with ADC = " << bemcData[daqId] <<" DAQ ID = "<<daqId<< endl;
        if (bemcData[daqId] >= fL0SeedThreshold) {
          L0Seeds.push_back(daqId);
	  cout << "Found L0Seed with ADC = " << bemcData[daqId] <<" DAQ ID = "<<daqId<< endl;
	}
      }
    }
    else {
...
void Quarkonium::calcCluster(int daqId)
{
  ...
  //
  float energy = (bemcData[daqId] - bemcTower[daqId].pedestal) * bemcTower[daqId].gain;
  float energies[8];
...
bool Quarkonium::checkClusterCtbHit(int daqId) const
{
...
float Quarkonium::bbcVertexZ() const
{
  //
...
void Quarkonium::readBemcStatus()
{
  //
...
#else
  string filename = Form("bemcStatus.txt.%s.%d", gSystem->HostName(), gSystem->GetPid());
#endif
  ifstream in(filename.c_str());
  if (!in) {
    fLogfile << "Can't open " << filename << endl;
...
void Quarkonium::readBemcCtbMap()
{
...
//
void Quarkonium::makeBemcTables()
{
  makeBemcGainTable();
...
void Quarkonium::makeBemcGainTable()
{
...
void Quarkonium::makeBemcHwPedestalTable()
{
...
void Quarkonium::makeBemcPedestalTable()
{
...
void Quarkonium::makeBemcKillTable()
{
...
void Quarkonium::makeBemcPositionTable()
{
...
void Quarkonium::makeBemcNeighborTable()
{
...
void Quarkonium::createHistograms()
{
...
void Quarkonium::writeHistograms()
{
...
void Quarkonium::resetHistograms()
{
...
struct deleteHistogram : public unary_function<Histogram*, Histogram*> {
  ...
  {
    cout << " Deleting " << h->name() << ", " << h->title() << endl;
    delete h;
    h=0;
    return 0;
  }
...
void Quarkonium::deleteHistograms()
{
  cout << "Quarkonium::deleteHistograms is being called!!!!" << endl;
  transform(fHistograms.begin(), fHistograms.end(), fHistograms.begin(), deleteHistogram());
}

void Quarkonium::print()
{
  const int L2RESULT_OFFSET = (name() == "ups") ? L2RESULTS_OFFSET_UPS : L2RESULTS_OFFSET_JPSI;
...
void Quarkonium::setBaseFileName(int runNumber)
{
  ostringstream s;
...
string Quarkonium::timeString() const
{
  time_t t = time(0);
  return ctime(&t);
}
...
void Quarkonium::readBtowDbFile()
{
  ...
  while (getline(in, line)) {
    ...
    int   m, s, e;
    int   daqId;
    scanf(line.c_str(), "%s %x %x %d %c %d %f %f %f %x %x id%d-%d-%d-%d %d",
...
void Quarkonium::readTowerMaskFile()
{
...
void Quarkonium::readBemcKillTable()
{
...
void Quarkonium::readCtbKillTable()
{
