...
void calcRaa_sym() {
  ...
  cout << "npart is " << npart29 << " " << npart25 << " " << npart57 << " " << npart79 << endl;
	...
	double effi123S[4], effi123SL[4], effi123ST[4];
	string t; int lc = 0;
...
// build systematics from sources here

  const int nsources = 6;
  fstream sysfeffiLM1S("data/sys_effiLM1S.txt");
  fstream sysfeffiLM2S("data/sys_effiLM2S.txt");
  fstream sysfeffiLM3S("data/sys_effiLM3S.txt");
  fstream sysfeffiH1S("data/sys_effiH1S.txt");
  fstream sysfeffiH2S("data/sys_effiH2S.txt");
  fstream sysfeffiH3S("data/sys_effiH3S.txt");

  double sysIndivLM1S[nsources][4], sysIndivH1S[nsources][4], sysIndiv1S[nsources][4];
  double sysIndivLM2S[nsources][4], sysIndivH2S[nsources][4], sysIndiv2S[nsources][4];
  double sysIndivLM3S[nsources][4], sysIndivH3S[nsources][4], sysIndiv3S[nsources][4];
  double sysIndiv23S[nsources][4];
  double sysIndiv123S[nsources][4];

  lc = 0;
  while ( !sysfeffiLM1S.eof() && lc < 4*nsources ) {
		getline(sysfeffiLM1S,t);
		for (int iS = 0; iS<nsources; iS++) if (lc%nsources==iS) sysIndivLM1S[iS][lc/nsources] = atof(t.c_str());
		lc++; }
  lc = 0;
  while ( !sysfeffiLM2S.eof() && lc < 4*nsources ) {
    	getline(sysfeffiLM2S,t);
    	for (int iS = 0; iS<nsources; iS++) if (lc%nsources==iS) sysIndivLM2S[iS][lc/nsources] = atof(t.c_str());
      	lc++; }
  lc = 0;
  while ( !sysfeffiLM3S.eof() && lc < 4*nsources ) {
    	getline(sysfeffiLM3S,t);
    	for (int iS = 0; iS<nsources; iS++) if (lc%nsources==iS) sysIndivLM3S[iS][lc/nsources] = atof(t.c_str());
      	lc++; }
  lc = 0;
  while ( !sysfeffiH1S.eof() && lc < 4*nsources ) {
    	getline(sysfeffiH1S,t);
    	for (int iS = 0; iS<nsources; iS++) if (lc%nsources==iS) sysIndivH1S[iS][lc/nsources] = atof(t.c_str());
      	lc++; }
  lc = 0;
  while ( !sysfeffiH2S.eof() && lc < 4*nsources ) {
    	getline(sysfeffiH2S,t);
    	for (int iS = 0; iS<nsources; iS++) if (lc%nsources==iS) sysIndivH2S[iS][lc/nsources] = atof(t.c_str());
      	lc++; }
  lc = 0;
  while ( !sysfeffiH3S.eof() && lc < 4*nsources ) {
    	getline(sysfeffiH3S,t);
    	for (int iS = 0; iS<nsources; iS++) if (lc%nsources==iS) sysIndivH3S[iS][lc/nsources] = atof(t.c_str());
      	lc++; }
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		{
			sysIndiv1S[k][i] = coffLM*sysIndivLM1S[k][i] + coffH*sysIndivH1S[k][i];
			sysIndiv2S[k][i] = coffLM*sysIndivLM2S[k][i] + coffH*sysIndivH2S[k][i];
			sysIndiv3S[k][i] = coffLM*sysIndivLM3S[k][i] + coffH*sysIndivH3S[k][i];
			sysIndiv23S[k][i] = 0.689*sysIndiv2S[k][i] + 0.311*sysIndiv3S[k][i];

			printf("cent: %i sysInd1S %i is %.3f \n", i, k, sysIndiv1S[k][i]);
			printf("cent: %i sysInd23S %i is %.3f \n", i, k, sysIndiv23S[k][i]);
		}
	}
	...
	while ( !fyields.eof() && lc < 16 )	{
		...
		cout << "went here a" << endl;

	for (int i = 0; i < 4; ++i)
	{
		yield123S[i] = yield1S[i] + yield23S[i];
		coeff1S[i] = yield1S[i]/yield123S[i];
		coeff23S[i] = yield23S[i]/yield123S[i];
//		err123S[i] = coeff1S[i] * err1S[i] + coeff23S[i]*err23S[i];
		err123S[i] = sqrt ( err1S[i]*err1S[i] + err23S[i]*err23S[i]);//coeff1S[i] * err1S[i] + coeff23S[i]*err23S[i];


		cout << "went here aa" << endl;

		effi123S[i] 	= coeff1S[i]*effi1S[i]+coeff23S[i]*effi23S[i];
		effi123SL[i] = coeff1S[i]*effi1SL[i]+coeff23S[i]*effi23SL[i];
		effi123ST[i] = coeff1S[i]*effi1ST[i]+coeff23S[i]*effi23ST[i];

		for (int k = 0; k < nsources; ++k)
		{
			sysIndiv123S[k][i] = coeff1S[i]*sysIndiv1S[k][i] + coeff23S[i]*sysIndiv23S[k][i];
		}
	}
	...
	double raa123S[4];
	double raa123Sstat[4];
	double raa123SsysL[4];
	double raa123SsysT[4];
	double raa123Sncol[4];

	double dcent[] = {0.6, 0.3, 0.2, 0.1};
	double sharedErr = 0.08*0.08 + 0.116*0.116 + 0.03*0.03;
	double furtherL[4] = {sqrt(0.0488*0.0488), sqrt(0.025*0.025), sqrt(0.061*0.061), sqrt(0.082*0.082)};
	double furtherT[4] = {sqrt(0.0488*0.0488), sqrt(0.025*0.025), sqrt(0.061*0.061), sqrt(0.082*0.082)};
	double further23[4] = {sqrt(0.128*0.128), sqrt(0.126*0.126), sqrt(0.192*0.192), sqrt(0.178*0.178)};
	

	double allSys1SL[4], allSys1ST[4];
	double allSys23SL[4], allSys23ST[4];
	double allSys123SL[4], allSys123ST[4];
	for (int i = 0; i < 4; ++i)
	{
		allSys1SL[i] = sharedErr + furtherL[i]*furtherL[i];
		allSys1ST[i] = sharedErr + furtherT[i]*furtherT[i] + 0.0091;
		allSys23SL[i] = sharedErr + further23[i]*further23[i];
		allSys23ST[i] = sharedErr + further23[i]*further23[i] + 0.0091;
		for (int k = 0; k < nsources; ++k)
		{
			allSys1SL[i] += sysIndiv1S[k][i]*sysIndiv1S[k][i];
			allSys23SL[i] += sysIndiv23S[k][i]*sysIndiv23S[k][i];
			allSys1ST[i] += sysIndiv1S[k][i]*sysIndiv1S[k][i];
			allSys23ST[i] += sysIndiv23S[k][i]*sysIndiv23S[k][i];
		}
		allSys1SL[i] = sqrt(allSys1SL[i]);
		allSys1ST[i] = sqrt(allSys1ST[i]);
		allSys23SL[i] = sqrt(allSys23SL[i]);
		allSys23ST[i] = sqrt(allSys23ST[i]);

		allSys123SL[i] = coeff1S[i]*allSys1SL[i] + coeff23S[i]*allSys23SL[i];
		allSys123ST[i] = coeff1S[i]*allSys1ST[i] + coeff23S[i]*allSys23ST[i];
	}
	...
	for (int iC = 0; iC < 4; ++iC)
	{
		...
		raa1SsysL[iC] = raa1S[iC]*allSys1SL[iC];
		raa1SsysT[iC] = raa1S[iC]*allSys1ST[iC];
	  	...
	  	ny1SsysL[iC] = ny1S[iC]*allSys1SL[iC];
	  	ny1SsysT[iC] = ny1S[iC]*allSys1ST[iC];
		...
		raa23SsysL[iC] = raa23S[iC]*allSys23SL[iC];
		raa23SsysT[iC] = raa23S[iC]*allSys23ST[iC];

	  	ny23S[iC] = yield23S[iC]*nEvQuoted/(lumi*nEvSeen*effi23S[iC]*sigmaAA*dcent[iC]*dy);
	  	ny23Sstat[iC] = err23S[iC]*nEvQuoted/(lumi*nEvSeen*effi23S[iC]*sigmaAA*dcent[iC]*dy);
	  	ny23SsysL[iC] = ny23S[iC]*allSys23SL[iC];
	  	ny23SsysT[iC] = ny23S[iC]*allSys23ST[iC];


		raa123S[iC] = yield123S[iC]*sigmaPP*nEvQuoted/(lumi*nEvSeen*effi123S[iC]*sigmaAA*dcent[iC]*dsigPP*nbin[iC]*dy);
		raa123Sstat[iC] = err123S[iC]*sigmaPP*nEvQuoted/(lumi*nEvSeen*effi123S[iC]*sigmaAA*dcent[iC]*dsigPP*nbin[iC]*dy);

		raa123SsysL[iC] = raa123S[iC]*allSys123SL[iC];
		raa123SsysT[iC] = raa123S[iC]*allSys123ST[iC];

	  	ny123S[iC] = yield123S[iC]*nEvQuoted/(lumi*nEvSeen*effi123S[iC]*sigmaAA*dcent[iC]*dy);
	  	ny123Sstat[iC] = err123S[iC]*nEvQuoted/(lumi*nEvSeen*effi123S[iC]*sigmaAA*dcent[iC]*dy);
	  	ny123SsysL[iC] = ny123S[iC]*allSys123SL[iC];
	  	ny123SsysT[iC] = ny123S[iC]*allSys123ST[iC];
...
//		raa23SsysT[iC] = sqrt( (raa23S[iC]-yieldE)*(raa23S[iC]-yieldE) + 4*raa23S[iC]*furtherT[iC]*raa23S[iC]*furtherT[iC]);
		...
		printf("raa 123s is : %f +- %f \n", raa123S[iC], raa123Sstat[iC]);
	}

	cout << "INTLUMI IS " << lumi*nEvSeen/nEvQuoted << endl;
	...
	pEff->SetTitle(";;total efficiency");
	...
	TCanvas* c1s = new TCanvas("c1s","",1200,880);
	double raaWid[] = {7., 7., 7., 7.};
	double raaWid2[] = {0, 0, 0, 0};
	TGraphAsymmErrors* pRaa1S = new TGraphAsymmErrors(1,npart,raa1S,raaWid,raaWid,raa1Sstat,raa1Sstat);
	TGraphAsymmErrors* pRaa1S_2 = new TGraphAsymmErrors(3,&npart[1],&raa1S[1],raaWid2,raaWid2,&raa1Sstat[1],&raa1Sstat[1]);
	TGraphAsymmErrors* pRaa1S_sys = new TGraphAsymmErrors(3,&npart[1],&raa1S[1],raaWid,raaWid,&raa1SsysL[1],&raa1SsysT[1]);
	TGraphAsymmErrors* pRaa1S_ncol = new TGraphAsymmErrors(3,&npart[1],&raa1S[1],raaWid,raaWid,&raa1Sncol[1],&raa1Sncol[1]);
	...
	xax = pRaa1S_2->GetXaxis();
	yax = pRaa1S_2->GetYaxis();
	//xax->CenterTitle();
	...
	xax->SetLimits(-10.,400.);
	pRaa1S_2->SetMinimum(0.0);
	pRaa1S_2->SetMaximum(1.4);
	...
	pRaa1S_2->SetMarkerSize(-4.0);
	pRaa1S_2->SetMarkerStyle(29);
	pRaa1S_2->SetMarkerColor(kBlue-7);
	pRaa1S_2->SetLineColor(kBlue-7);
	...
	pRaa1S_2->Draw("PZ");
	pRaa1S_2->Draw("PZ");
	pRaa1S_sys->SetLineColor(kBlue-7);
	pRaa1S_sys->SetLineWidth(1);
	pRaa1S_sys->SetFillColor(kBlue-7);
	...
	comb1Sstat->SetMarkerColor(kBlue);
	comb1Ssys->SetFillColor(2);
	comb1Ssys->SetFillStyle(2001);
	comb1Ssys->SetLineColor(2);
	//comb1Ssys->Draw("P2");
	//comb1Sstat->Draw("P");
	...
	c1s->SaveAs("plots/my1s.png");

	// 23S

	
	TCanvas* c23s = new TCanvas("c1s","",1200,880);
	TGraphAsymmErrors* pRaa23S = new TGraphAsymmErrors(1,npart,raa23S,raaWid,raaWid,raa23Sstat,raa23Sstat);
	TGraphAsymmErrors* pRaa23S_2 = new TGraphAsymmErrors(3,&npart[1],&raa23S[1],raaWid2,raaWid2,&raa23Sstat[1],&raa23Sstat[1]);
	TGraphAsymmErrors* pRaa23S_sys = new TGraphAsymmErrors(3,&npart[1],&raa23S[1],raaWid,raaWid,&raa23SsysL[1],&raa23SsysT[1]);
	TGraphAsymmErrors* pRaa23S_ncol = new TGraphAsymmErrors(4,&npart[1],&raa23S[1],raaWid,raaWid,&raa23Sncol[1],&raa23Sncol[1]);
...
xax = pRaa23S_2->GetXaxis();
	yax = pRaa23S_2->GetYaxis();
	//xax->CenterTitle();
	xax->SetLabelSize(0.04);
	xax->SetTitleSize(0.05);
	xax->SetTitleOffset(+0.9);
	...
	yax->SetTitleOffset(+0.8);
	xax->SetLimits(-10.,400.);
	pRaa23S_2->SetMinimum(0.0);
	pRaa23S_2->SetMaximum(1.4);
	...
	pRaa23S_2->SetMarkerSize(-4.0);
	pRaa23S_2->SetMarkerStyle(29);
	pRaa23S_2->SetMarkerColor(kBlue-7);
	pRaa23S_2->SetLineColor(kBlue-7);
	...
	pRaa23S_2->Draw("PZ");
	pRaa23S_2->Draw("PZ");
	pRaa23S_sys->SetLineColor(kBlue-7);
	pRaa23S_sys->SetLineWidth(1);
	pRaa23S_sys->SetFillColor(kBlue-7);
	...
	comb2S3Sstat->SetMarkerColor(kBlue);
	comb2S3Ssys->SetFillColor(2);
	comb2S3Ssys->SetFillStyle(2001);
	comb2S3Ssys->SetLineColor(2);
	//comb2S3Ssys->Draw("P2");
	//comb2S3Sstat->Draw("P");
	...
	c23s->SaveAs("plots/my2s.png");


	//123

	TCanvas* c123s = new TCanvas;
	TGraphAsymmErrors* pRaa123S = new TGraphAsymmErrors(1,npart,raa123S,raaWid,raaWid,raa123Sstat,raa123Sstat);
	TGraphAsymmErrors* pRaa123S_2 = new TGraphAsymmErrors(3,&npart[1],&raa123S[1],raaWid,raaWid,&raa123Sstat[1],&raa123Sstat[1]);
	TGraphAsymmErrors* pRaa123S_sys = new TGraphAsymmErrors(4,npart,raa123S,raaWid,raaWid,raa123SsysL,raa123SsysT);
	//TGraphAsymmErrors* pRaa123S_ncol = new TGraphAsymmErrors(4,npart,raa123S,raaWid,raaWid,raa123Sncol,raa123Sncol);
	TGraphAsymmErrors* pRaa123S_norm = new TGraphAsymmErrors(1,normX,normY,raaWid,raaWid,normEL,normET);
	pRaa123S->SetTitle(";<N_{part}>;R_{AA}");
	xax = pRaa123S->GetXaxis();
	yax = pRaa123S->GetYaxis();
	//xax->CenterTitle();
	xax->SetLabelSize(0.04);
	xax->SetTitleSize(0.05);
	xax->SetTitleOffset(+0.9);
	...
	yax->SetTitleOffset(+0.8);
	xax->SetLimits(0.,400.);
	pRaa123S->SetMinimum(0.0);
	pRaa123S->SetMaximum(1.4);
	pRaa123S->SetMarkerSize(2.2);
	pRaa123S->SetMarkerStyle(30);
	pRaa123S->SetMarkerColor(2);
	pRaa123S->SetLineColor(2);
	pRaa123S->SetLineWidth(1);
	pRaa123S->Draw("AP");
	//pRaa123S_ncol->SetLineColor(kGray);
	//pRaa123S_ncol->SetLineWidth(1);
	//pRaa123S_ncol->SetFillColor(kGray);
	//pRaa123S_ncol->SetFillStyle(1001);
	//pRaa123S_ncol->Draw("2");
	pRaa123S->Draw("P");
	pRaa123S_2->SetMarkerSize(2.2);
	pRaa123S_2->SetMarkerStyle(29);
	pRaa123S_2->SetMarkerColor(2);
	pRaa123S_2->SetLineColor(2);
	pRaa123S_2->SetLineWidth(1);
	pRaa123S_2->Draw("P");
	pRaa123S_sys->SetLineColor(2);
	pRaa123S_sys->SetLineWidth(1);
	pRaa123S_sys->SetFillColor(2);
	pRaa123S_sys->SetFillStyle(2001);
	pRaa123S_sys->Draw("2");
	pRaa123S_norm->SetLineColor(2);
	pRaa123S_norm->SetLineWidth(1);
	pRaa123S_norm->SetFillColor(2);
	pRaa123S_norm->SetFillStyle(3001);
	pRaa123S_norm->Draw("2");
	...
	TLegend* lraa123S = new TLegend(0.13,0.69,0.41,0.88);
	lraa123S->SetNColumns(1);
	lraa123S->SetFillStyle(0);
	lraa123S->SetBorderSize(0);
	lraa123S->SetTextSize(0.032);
	lraa123S->AddEntry((TObject*)0,"#bf{Au+Au 2014 #sqrt{s_{NN}} = 200 GeV}"," ");
	lraa123S->AddEntry(pRaa123S_2,"#Upsilon(1S+2S+3S)->e^{+}e^{-}, |y|<0.5, 60-30%, 30-10%, 10-0%","p");
	lraa123S->AddEntry(pRaa123S,"0-60%","p");
	//lraa123S->AddEntry(pRaa123S_sys,"systematic uncertainty","f");
	//lraa123S->AddEntry(pRaa123S_ncol,"STAR N_{coll} uncertainty","f");
	lraa123S->Draw();


	c123s->SaveAs("raa123s.eps");
	...
	TGraphAsymmErrors* pNY1S = new TGraphAsymmErrors(1,npart,ny1S,raaWid,raaWid,ny1Sstat,ny1Sstat);
	TGraphAsymmErrors* pNY1S_2 = new TGraphAsymmErrors(3,&npart[1],&ny1S[1],raaWid,raaWid,&ny1Sstat[1],&ny1Sstat[1]);
	TGraphAsymmErrors* pNY1S_sys = new TGraphAsymmErrors(4,npart,ny1S,raaWid,raaWid,ny1SsysL,ny1SsysT);

	TGraphAsymmErrors* pNY23S = new TGraphAsymmErrors(1,npart,ny23S,raaWid,raaWid,ny23Sstat,ny23Sstat);
	TGraphAsymmErrors* pNY23S_2 = new TGraphAsymmErrors(3,&npart[1],&ny23S[1],raaWid,raaWid,&ny23Sstat[1],&ny23Sstat[1]);
	TGraphAsymmErrors* pNY23S_sys = new TGraphAsymmErrors(4,npart,ny23S,raaWid,raaWid,ny23SsysL,ny23SsysT);
	
	
	TFile* fout = new TFile("ups_data.root","recreate");
	pRaa1S->SetName("raa1Sstat_all");
	pRaa1S->Write();
	pRaa1S_2->SetName("raa1Sstat_cent");
	pRaa1S_2->Write();
	pRaa1S_sys->SetName("raa1Ssys");
	pRaa1S_sys->Write();
	pRaa23S->SetName("raa23Sstat_all");
	pRaa23S->Write();
	pRaa23S_2->SetName("raa23Sstat_cent");
	pRaa23S_2->Write();
	pRaa23S_sys->SetName("raa23Ssys");
	pRaa23S_sys->Write();

	pNY1S->SetName("nyield1Sstat_all");
	pNY1S->Write();
	pNY1S_2->SetName("nyield1Sstat_cent");
	pNY1S_2->Write();
	pNY1S_sys->SetName("nyield1Ssys");
	pNY1S_sys->Write();

	pNY23S->SetName("nyield23Sstat_all");
	pNY23S->Write();
	pNY23S_2->SetName("nyield23Sstat_cent");
	pNY23S_2->Write();
	pNY23S_sys->SetName("nyield23Ssys");
	pNY23S_sys->Write();

	ofstream outfile;
	outfile.open("1SsysSources.txt");
	for (int i = 0; i < 4; ++i)
	{
		...
		outfile << 0.03 << endl;
		outfile << 0.08 << endl;
		outfile << furtherL[i] << endl;
		for (int k = 0; k < nsources; ++k)
		{
			outfile << sysIndiv1S[k][i] << endl;
		}
	}
	outfile.close();
	...
	ofstream outfile2S;
	outfile2S.open("23SsysSources.txt");
	for (int i = 0; i < 4; ++i)
	{
		
		outfile2S << 0.116 << endl;
		outfile2S << 0.151 << endl;
		outfile2S << 0.03 << endl;
		outfile2S << 0.08 << endl;
		outfile2S << further23[i] << endl;
		for (int k = 0; k < nsources; ++k)
		{
			outfile2S << sysIndiv23S[k][i] << endl;
		}
	}
	outfile2S.close();


	ofstream outfile123S;
	outfile123S.open("raa123.txt");
	for (int i = 0; i < 4; ++i)
	{
		outfile123S << raa123S[i] << endl;
		outfile123S << raa123Sstat[i] << endl;
		outfile123S << raa123SsysL[i] << endl;
		outfile123S << raa123SsysT[i] << endl;
	}
	outfile123S.close();


	ofstream outny123S;
	outny123S.open("ny123.txt");
	for (int i = 0; i < 4; ++i)
	{
		outny123S << ny123S[i] << endl;
		outny123S << ny123Sstat[i] << endl;
		outny123S << ny123SsysL[i] << endl;
		outny123S << ny123SsysT[i] << endl;
	}
	outny123S.close();
